...
void merge(long n, T left[n], T right[n], T result[n*2], long start, long length, int depth) {
   if (length < MIN_MERGE_SIZE*2L) {
       ...
       if (!omp_in_final()) {
               ...
               merge(n, left, right, result, start, length/2, CUTOFF);
               merge(n, left, right, result, start + length/2, length/2, CUTOFF);
       }
   }
}
...
void multisort(long n, T data[n], T tmp[n], int depth) {
    if (n >= MIN_SORT_SIZE*4L) {
        ...
        if (!omp_in_final()) {
                #pragma omp task final(depth >= CUTOFF) depend(out:tmp[0])
               multisort(n/4L, &data[0], &tmp[0], depth+1);
               #pragma omp task final(depth >= CUTOFF) depend(out:tmp[n/4L])
               ...
               #pragma omp task final(depth >= CUTOFF) depend(out:tmp[3L*n/4L])
               multisort(n/4L, &data[3L*n/4L], &tmp[3L*n/4L], depth+1);

               #pragma omp task final(depth >= CUTOFF) depend(inout:tmp[0]) depend(in:tmp[n/4L])
               merge(n/4L, &data[0], &data[n/4L], &tmp[0], 0, n/2L, depth+1);
               #pragma omp task final(depth >= CUTOFF) depend(inout:tmp[n/2L]) depend(in:tmp[3L*n/4L])
